...
void Red_Headquarter::produce()
{
	...
	cout.width(3);//设置域宽
	cout << Time;
	cout<< ' ' << "red " << Name[temp] << ' ' << num << " born with strength " << life_[temp] << ',' << NumbersOfSodiers[temp] << ' ' << Name[temp] << " in red headquarter" << endl;
...
void Blue_Headquarter::produce()
{
	...
	//printf("%3d red %s %d born with strength %d,%d %s in red headquarter", &Time, &Name[temp], &num, &life_[temp], &NumbersOfSodiers[temp], &Name[temp]);
	Life_yuan -= life_[temp];
}
/////////////////////////////////////////////////// 以上为蓝方
		...
		while (red_can_produce || blue_can_produce)
		{
			...
			if (red.check_stop())
			{
				if (red_can_produce)
				{
					...
					//printf("%3d red headquarter stops making warriors", &Time);
			...
			if (blue.check_stop())
			{
				if (blue_can_produce)
				{
					...
					//printf("%3d blue headquarter stops making warriors", &Time);
